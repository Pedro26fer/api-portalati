import { Controller, Post, Get, Patch, Param, Delete, Body, UseGuards } from "@nestjs/common";
...
export class EntidadeController{
    ...
    async createEntidade(@Body() createEntidadeDto: CreateEntidadeDto): Promise<Entidade>{
    ...
    @Patch('update/:id')
    @UseGuards(JwtAuthGuard)
    async update(@Param('id') id: string, @Body() updateEntidadeDto: UpdateEntidadeDto): Promise<Entidade>{
